...
if(empty($name) || empty($lastname) || empty($username) || empty($email) || empty($date) || empty($nationaliteit) || empty($adres) || empty($cadres) || empty($telnr) || empty($gender) || empty($password) || empty($passwordRepeat)) {
  ...
  /*else if ($pin !== $pinRepeat) {
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      ...
      if ($resultCheck > 0) {
        ...
        $sql = "INSERT INTO gegevens (NAAM, ACHTERNAAM, USERNAME, EMAIL, PWD, GEBDATUM, NATIONALITEIT, WOONADRES, TELNUMMER, CORRESPONDENTIEADRES, GESLACHT) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_stmt_init($conn);
        if (!mysqli_stmt_prepare($stmt, $sql)) {
         ...
         // $hashedPwd = password_hash($password, PASSWORD_DEFAULT);
          ...
          mysqli_stmt_bind_param($stmt, "sssssssssss", $name, $lastname, $username, $email, $password, $date, $nationaliteit, $adres, $telnr, $cadres, $gender);
          mysqli_stmt_execute($stmt);
